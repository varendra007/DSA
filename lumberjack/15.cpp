...
int allTrees[1002][1002], grid_size;
int check[1002][1002];
...
int profit_calculator()
{
  ...
  for (int i = 0; i < grid_size; i++)
  {
    for (int j = 0; j < grid_size; j++)
    {
      ...
      {
        allTrees1[i][j].time = time_calculator(x_cur, y_cur, i, j, allTrees1[i][j].thickness);
...
int main()
{
  ...
  int min_time = Trees[0].time;
  int index_ = 0;
  for (int i = 0; i < tot_trees; i++)
  {
    if (Trees[i].time < min_time)
    {
      min_time = Trees[i].time;
      index_ = i;
    }
  }
  if (Trees[index_].x + Trees[index_].y + Trees[index_].thickness <= total_time_left)
  {
    first_value = first_value + Trees[index_].value;
  }
  ...
  if (first_value == 40)
  {
    ...
    // print_route(x_cur, y_cur, Trees[index_].x, Trees[index_].y, 'd');
    ...
    // total_time_left -= time_calculator(x_cur, y_cur, Trees[index_].x, Trees[index_].y, Trees[index_].thickness);
    // x_cur = Trees[index_].x;
    // y_cur = Trees[index_].y;
    // Trees[index_].x = -1;
    // Trees[index_].y = -1;

    return 0;
  }
  else if (first_value == 18)
  {
